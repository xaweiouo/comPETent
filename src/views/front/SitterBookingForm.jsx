...
function SitterBookingForm() {
  ...
  function handleBookingChange(e) {
    ...
    // 處理縣市變更：更新 city、清空 district、更新 districtOptions
    if (name === "pickup_city") {
    ...
    // 處理地區變更：同時算出對應的 location_id
    if (name === "pickup_district") {
      const selectedLoc = locations.find(
        (loc) =>
          loc.city === bookingForm.pickup_city && loc.district === value
      );
      ...
      setBookingForm((prev) => {
        const nextForm = {
          ...
          pickup_location_id: selectedLoc ? selectedLoc.id : null,
        };
  ...
  async function handleBookingSubmit(e) {
      ...
      .insert([
        {
          ...
          //新增地區
          pickup_location_id: bookingForm.pickup_location_id,
    ...
    dispatch(
      createAsyncMessage({
        text: `建立預約成功！訂單編號：${insertData.order_number}`,
      })
    );
                    ...
                    <div className="col-12 col-sm-4">
                      <div className="input-group rounded-pill overflow-hidden border border-warning">
                        <select
                          ...
                          value={bookingForm.pickup_district}
                          onChange={handleBookingChange}
                          disabled={!bookingForm.pickup_city}
                        >
                          <option value="">
                            {bookingForm.pickup_city ? "請選擇地區" : "請先選縣市"}
                          </option>
